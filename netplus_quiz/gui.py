...
COLORS = {
    "dark": {"bg": "#2E3440", "sidebar": "#242933", "fg": "#ECEFF4", "primary": "#0072CE", "accent": "#FF8200", "nord_blue": "#88C0D0", "correct": "#A3BE8C", "wrong": "#BF616A", "hover": "#3B4252"},
    "light": {"bg": "#ECEFF4", "sidebar": "#D8DEE9", "fg": "#2E3440", "primary": "#0072CE", "accent": "#FF8200", "nord_blue": "#5E81AC", "correct": "#4F734F", "wrong": "#8B4B4B", "hover": "#E5E9F0"}
}
...
class QuizApp(tk.Tk):
    ...
    def _apply_theme_colors(self) -> None:
        ...
        style.configure("TFrame", background=p["bg"]); style.configure("Sidebar.TFrame", background=p["sidebar"])
        style.configure("TLabel", background=p["bg"], foreground=p["fg"], font=self.ui_font)
        style.configure("Header.TLabel", font=self.header_font); style.configure("Title.TLabel", font=self.title_font)
        style.configure("TButton", font=self.ui_font, padding=6, background=p["sidebar"], foreground=p["fg"])
        style.map("TButton", background=[("active", p["hover"])], foreground=[("active", p["fg"])])
        style.configure("Primary.TButton", background=p["primary"], foreground="white")
        style.map("Primary.TButton", background=[("active", p["nord_blue"])], foreground=[("active", "white")])
        style.configure("Sidebar.TButton", background=p["sidebar"], foreground=p["fg"], borderwidth=0)
        style.map("Sidebar.TButton", background=[("active", p["bg"])], foreground=[("active", p["fg"])])
        style.configure("TCheckbutton", background=p["bg"], foreground=p["fg"], font=self.ui_font)
        style.map("TCheckbutton", background=[("active", p["bg"])], foreground=[("active", p["fg"])])
        style.configure("TRadiobutton", background=p["bg"], foreground=p["fg"], font=self.ui_font)
        style.map("TRadiobutton", background=[("active", p["bg"])], foreground=[("active", p["fg"])])
        style.configure("TProgressbar", thickness=10, background=p["nord_blue"])

    def _build_layout(self) -> None:
        self.sidebar = ttk.Frame(self, style="Sidebar.TFrame", width=200); self.sidebar.pack(side="left", fill="y"); self.sidebar.pack_propagate(False)
        ttk.Label(self.sidebar, text="Net+", style="Title.TLabel", background=self.palette["sidebar"]).pack(pady=20)
        nav_items = [("Dashboard", self._show_dashboard), ("Practice Quiz", self._show_quiz_config), ("Port Practice", self._show_port_config), ("PoE Practice", self._show_poe_config), ("WiFi Practice", self._show_wifi_config), ("Cabling Practice", self._show_cabling_config), ("Flashcards", self._show_fc_config), ("Subnetting", self._show_subnetting), ("Study Notes", self._show_notes), ("Settings", self._show_settings)]
        for text, cmd in nav_items: ttk.Button(self.sidebar, text=text, command=lambda c=cmd: self._safe_nav(c), style="Sidebar.TButton").pack(fill="x", padx=10, pady=5)
        self.main_area = ttk.Frame(self); self.main_area.pack(side="right", fill="both", expand=True)
    ...
    def _render_quiz_view(self) -> None:
        ...
        choices = self.session.current_choices if self.session.is_review else q.choices
    ...
    def _submit_quiz(self) -> None:
        ...
        if "drill" not in q.id and not self.session.is_review: save_performance(q.id, rec.is_correct)
    ...
    def _show_subnetting(self) -> None:
        ...
        for label in ["Mask", "Network", "Broadcast"]:
            f = ttk.Frame(frame); f.pack(fill="x", pady=5); ttk.Label(f, text=f"{label}:", width=15).pack(side="left"); e = tk.Entry(f, font=self.ui_font, bg=self.palette["sidebar"], fg=self.palette["fg"], insertbackground=self.palette["fg"], bd=0, highlightthickness=1); e.config(highlightbackground=self.palette["sidebar"], highlightcolor=self.palette["primary"]); e.pack(side="left", fill="x", expand=True); self.entries[label.lower()] = e; cl = ttk.Label(f, text="", foreground=self.palette["nord_blue"]); cl.pack(side="left", padx=10); self.correct_labels[label.lower()] = cl
        ttk.Button(frame, text="Check", style="Primary.TButton", command=lambda: self._check_subnet(chal)).pack(pady=20); self.sub_fb = ttk.Label(frame, text=""); self.sub_fb.pack(); ttk.Button(frame, text="New", command=self._show_subnetting).pack()

    def _check_subnet(self, chal: dict) -> None:
        correct = True
        for k, e in self.entries.items():
            u = e.get().strip(); a = chal[k]
            if u == a: e.config(highlightbackground=self.palette["correct"], highlightcolor=self.palette["correct"]); self.correct_labels[k].config(text="")
            else: e.config(highlightbackground=self.palette["wrong"], highlightcolor=self.palette["wrong"]); self.correct_labels[k].config(text=f"Expected: {a}"); correct = False
        self.sub_fb.config(text="Perfect!" if correct else "Review red fields.", foreground=self.palette["correct"] if correct else self.palette["wrong"])

    def _show_settings(self) -> None:
        self._clear_main(); frame = ttk.Frame(self.main_area, padding=40); frame.pack(fill="both", expand=True); ttk.Label(frame, text="Settings", style="Title.TLabel").pack(anchor="w", pady=(0, 20)); ttk.Label(frame, text="Base Font Size:").pack(anchor="w"); fs_var = tk.IntVar(value=self.app_settings["font_size"]); s = ttk.Scale(frame, from_=8, to=24, variable=fs_var, orient="horizontal"); s.pack(fill="x", pady=10); theme_var = tk.StringVar(value=self.current_theme); ttk.Radiobutton(frame, text="Nord Dark", value="dark", variable=theme_var).pack(anchor="w"); ttk.Radiobutton(frame, text="Nord Light", value="light", variable=theme_var).pack(anchor="w"); ttk.Button(frame, text="Save & Restart", style="Primary.TButton", command=lambda: self._save_settings_logic(fs_var.get(), theme_var.get())).pack(pady=20); ttk.Separator(frame).pack(fill="x", pady=20); ttk.Button(frame, text="Reset Progress Data", command=self._reset_data_logic).pack()

    def _save_settings_logic(self, fs: int, theme: str) -> None: self.app_settings.update({"font_size": fs, "theme": theme}); save_settings(self.app_settings); messagebox.showinfo("Restart", "Please restart the application to apply changes.")
    def _reset_data_logic(self) -> None:
        if messagebox.askyesno("Confirm", "Are you sure?"): reset_all_data(); self._show_dashboard()

    def _show_notes(self, specific: Path = None) -> None:
        win = tk.Toplevel(self); win.title("Study Notes"); win.geometry("800x600"); win.configure(bg=self.palette["bg"]); txt = ScrolledText(win, bg=self.palette["bg"], fg=self.palette["fg"], font=self.ui_font, padx=10, pady=10); txt.pack(fill="both", expand=True)
        if specific: content = specific.read_text(encoding="utf-8")
        else:
            p_list = sorted(NOTES_DIR.glob("*.txt")); content = "\n".join([f"=== {p.name} ===\n{p.read_text()}\n" for p in p_list if not p.name.endswith(".json")])
